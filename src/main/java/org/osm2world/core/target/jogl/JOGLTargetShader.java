...
public class JOGLTargetShader extends AbstractJOGLTarget implements JOGLTarget {
	...
	@Override
	public void drawBackgoundImage(File backgroundImage, int startPixelX,
								   int startPixelY, int pixelWidth,
								   int pixelHeight,
		...
		FloatBuffer valueBuffer =
			Buffers.newDirectFloatBuffer(verticeCount * (2 * valueCount));
		valueBuffer.put(0);
		valueBuffer.put(0);
		...
		int stride = 0;
		gl.glVertexAttribPointer(backgroundShader.getVertexPositionID(),
								 valueCount, GL.GL_FLOAT, false, stride, 0);
		gl.glVertexAttribPointer(
			backgroundShader.getVertexTexCoordID(), valueCount, GL.GL_FLOAT,
			false, stride, Buffers.SIZEOF_FLOAT * valueCount * verticeCount);
	...
	/**
	 * Calculate tighter near and far planes for the boundingBox around the
	...
	updateClippingPlanesForCamera(Camera camera, Projection projection,
		...
		camMat.gluLookAt((float)pos.x, (float)pos.y, (float)-pos.z,
		...
		for (VectorXYZ corner : boundingBox.corners()) {
			...
			FloatUtil.multMatrixVecf(camMat.glGetMvMatrixf(),
									 new float[] {(float)corner.x,
												  (float)corner.y,
												  (float)corner.z, 1},
									 result);
			VectorXYZ cornerCam =
				new VectorXYZ(result[0] / result[3], result[1] / result[3],
							  result[2] / result[3]);
		...
		if (nearPlane == Double.POSITIVE_INFINITY)
			...
			nearPlane =
				Math.max(projection.getNearClippingDistance(), nearPlane);
		if (farPlane == 0)
		...
		return new Projection(
			projection.isOrthographic(), projection.getAspectRatio(),
			projection.getVertAngle(), projection.getVolumeHeight(), nearPlane,
			farPlane);
	}
	...
	public void renderPart(Camera camera, Projection projection, double xStart,
		...
		if (renderingParameters.overwriteProjectionClippingPlanes) {
			projection = updateClippingPlanesForCamera(
				camera, projection, rendererShader.getBoundingBox());
		}

		applyProjectionMatricesForPart(pmvMatrix, projection, xStart, xEnd,
									   yStart, yEnd);
		...
		if (renderingParameters.useShadowMaps) {
			...
			shadowMapShader.setCameraFrustumPadding(
				renderingParameters.shadowMapCameraFrustumPadding);
			shadowMapShader.setShadowMapSize(
				renderingParameters.shadowMapWidth,
				renderingParameters.shadowMapHeight);
			shadowMapShader.useShader();
			shadowMapShader.preparePMVMatrix(globalLightingParameters,
											 pmvMatrix,
											 rendererShader.getBoundingBox());
			// render opaque shadow casters only when not using shadow volumes
			// simultaneously, as those will be rendered there
			shadowMapShader.setRenderOpaque(
				!renderingParameters.useShadowVolumes);
			...
			// ShaderManager.saveDepthBuffer(new
			// File("/home/sebastian/shadowmap"+xStart+"_"+yStart+".png"),
			// shadowMapShader.getShadowMapHandle(),
			// shadowMapShader.shadowMapWidth, shadowMapShader.shadowMapHeight,
			// gl); shadowMapShader.saveShadowMap(new
			// File("/home/sebastian/shadowmap.bmp"));
			// shadowMapShader.saveColorBuffer(new
			// File("/home/sebastian/shadowmap_color"+xStart+"_"+yStart+".png"));
		...
		if (!showShadowPerspective && renderingParameters.useSSAO) {
			defaultShader.enableSSAOwithDepthMap(
				ssaoShader.getDepthBuferHandle());
		}

		// if using shadow volumes render semi-transparent objects later
		defaultShader.setRenderSemiTransparent(
			!renderingParameters.useShadowVolumes);
		...
		if (renderingParameters.useShadowVolumes) {
			...
			/* Render shadow volumes with depth-fail algorithm. Uses the
			 * previously filled depth buffer */
			int[] drawbuffer = new int[1];
			gl.glGetIntegerv(GL2GL3.GL_DRAW_BUFFER, drawbuffer, 0);
			gl.glDrawBuffer(GL.GL_NONE);
			gl.glEnable(GL.GL_STENCIL_TEST);
			gl.glDepthMask(false);
			gl.glEnable(GL3.GL_DEPTH_CLAMP); // used to clamp the infinity big
											 // shadow volumes
			...
			gl.glStencilOpSeparate(GL.GL_BACK, GL.GL_KEEP, GL.GL_INCR_WRAP,
								   GL.GL_KEEP);
			gl.glStencilOpSeparate(GL.GL_FRONT, GL.GL_KEEP, GL.GL_DECR_WRAP,
								   GL.GL_KEEP);
			...
			// if using shadow volumes render semi-transparent objects later
			shadowVolumeShader.setRenderSemiTransparent(
				!renderingParameters.useShadowVolumes);
			...
			gl.glStencilOpSeparate(GL.GL_BACK, GL.GL_KEEP, GL.GL_KEEP,
								   GL.GL_KEEP);
			gl.glStencilOpSeparate(GL.GL_FRONT, GL.GL_KEEP, GL.GL_KEEP,
								   GL.GL_KEEP);
			...
			// NOTE: results could be improved slightly, if the depth-fail
			// algorithm is executed here as well
			//       the result would be that the topmost semi-transparent pixel
			//       would receive shadow volume shadows (needs to be
			//       investigated, if the difference is noticeable in practice)
			defaultShader.useShader();
			defaultShader.loadDefaults();
			defaultShader.setPMVMatrix(pmvMatrix);
	...
	static final void
	applyRenderingParameters(GL3 gl, JOGLRenderingParameters parameters) {
	...
	static final void applyCameraMatrices(PMVMatrix pmvMatrix, Camera camera) {
		...
		VectorXYZ lookAt = camera.getLookAt();
		VectorXYZ up = camera.getUp();
		pmvMatrix.gluLookAt((float)pos.x, (float)pos.y, (float)-pos.z,
							(float)lookAt.x, (float)lookAt.y, (float)-lookAt.z,
							(float)up.x, (float)up.y, (float)-up.z);
	}

	static final void applyProjectionMatrices(PMVMatrix pmvMatrix,
	 ...
	 */
	static final void applyProjectionMatricesForPart(PMVMatrix pmvMatrix,
													 Projection projection,
		...
		if ((xStart != 0 || xEnd != 1 || yStart != 0 || yEnd != 1) &&
			...
			throw new IllegalArgumentException(
				"section rendering only supported "
		...
		if (projection.isOrthographic()) {
			...
			double volumeWidth =
				projection.getAspectRatio() * projection.getVolumeHeight();
	...
	static final void
	applyLightingParameters(DefaultShader shader,
	...
	//				for (int texLayer = 0; texLayer < texCoordLists.size();
	// texLayer++) { 					VectorXZ textureCoord =
	// texCoordLists.get(texLayer).get(i); 					if (i==0) {
	//						gl.glVertexAttrib2d(shader.getVertexTexCoordID(),
	...
	//			gl.glVertexAttrib3d(shader.getVertexPositionID(), v.x, v.y,
	//-v.z);
	//
	//		}
	//
	 ...
	 */
	protected final void drawBoundingBox(Color color,
										 AxisAlignedBoundingBoxXYZ bb) {
		// bottom
		drawBox(color, new VectorXYZ(bb.minX, bb.minY, bb.minZ),
				new VectorXYZ(bb.minX, bb.minY, bb.maxZ),
				new VectorXYZ(bb.maxX, bb.minY, bb.maxZ),
				new VectorXYZ(bb.maxX, bb.minY, bb.minZ));
		// top
		drawBox(color, new VectorXYZ(bb.minX, bb.maxY, bb.minZ),
	 ...
	 */
	protected final void drawLine(Color color, VectorXYZ v1, VectorXYZ v2) {
	 ...
	 */
	protected final void drawBox(Color color, VectorXYZ v1, VectorXYZ v2,
								 VectorXYZ v3, VectorXYZ v4) {
	...
	public void finish() {
		...
		// this.drawLineLoop(Color.WHITE, 1, Arrays.asList(new
		// VectorXYZ[]{xzBoundary.topLeft().xyz(0.1),
		// xzBoundary.topRight().xyz(0.1), xzBoundary.bottomRight().xyz(0.1),
		// xzBoundary.bottomLeft().xyz(0.1)}));
		rendererShader = new JOGLRendererVBOShader(gl, textureManager,
												   primitiveBuffer, xzBoundary);
		...
		nonAreaRenderer = new JOGLRendererVBONonAreaShader(gl, nonAreaShader,
														   nonAreaPrimitives);
		if (renderingParameters.useShadowVolumes)
			rendererShadowVolume = new JOGLRendererVBOShadowVolume(
				gl, primitiveBuffer,
				new VectorXYZW(globalLightingParameters.lightFromDirection, 0));
	}
